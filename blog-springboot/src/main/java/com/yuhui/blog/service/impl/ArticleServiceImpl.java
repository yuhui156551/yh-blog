...
import com.yuhui.blog.dao.TagDao;
...
import static com.yuhui.blog.constant.CommonConst.ARTICLE_SET;
import static com.yuhui.blog.constant.CommonConst.FALSE;
import static com.yuhui.blog.constant.RedisPrefixConst.*;
import static com.yuhui.blog.enums.ArticleStatusEnum.DRAFT;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleDao, Article> implements ArticleService {
    ...
    public PageResult<ArchiveDTO> listArchives() {
        Page<Article> page = new Page<>(PageUtils.getCurrent(), PageUtils.getSize());// 1,10
        // 分页查询数据（只获取文章id、文章name、文章创建时间）
    ...
    public PageResult<ArticleBackDTO> listArticleBacks(ConditionVO condition) {
        ...
        // 查询文章点赞量和浏览量
        Map<Object, Double> viewsCountMap = redisService.zAllScore(ARTICLE_VIEWS_COUNT);// 获取所有分数，分数即文章id
        Map<String, Object> likeCountMap = redisService.hGetAll(ARTICLE_LIKE_COUNT); // 获取整个hash结构
        // 封装点赞量和浏览量
    ...
    public List<String> exportArticles(List<Integer> articleIdList) {
        ...
        for (Article article : articleList) {
            ...
            try (ByteArrayInputStream inputStream = new ByteArrayInputStream(article.getArticleContent().getBytes())) {
                // 上传文件并返回文件访问路径
                String url = uploadStrategyContext.executeUploadStrategy(article.getArticleTitle() + FileExtEnum.MD.getExtName(), // "标题.md"
                        inputStream, FilePathEnum.MD.getPath());// "markdown/"
                urlList.add(url);
            } catch (Exception e) {
                // ExceptionUtil.stacktraceToString 堆栈转为完整字符串
